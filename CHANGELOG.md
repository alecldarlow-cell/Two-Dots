...
- Security advisor: 5 ERRORs cleared, leaving 3 expected `anon_security_definer_function_executable` WARNs (the kpi\_\* RPC functions are intentionally callable by anon — that's the public dashboard's whole point) and 2 pre-existing permissive-RLS WARNs on `devices` (unrelated, already there).
