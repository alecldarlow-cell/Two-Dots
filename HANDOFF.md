...
Three RPC functions over `analytics_events` (`SECURITY DEFINER`, `EXECUTE` granted to `anon`, `search_path = ''` pinned — see migration 005). They are the _only_ public read surface for analytics; the underlying table is service-role-read-only:
